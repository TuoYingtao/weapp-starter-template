 ...
 */
module.exports = {
  ignorePatterns: ['node_modules/', 'build/', 'lib.*.js', '.eslintrc.js'],
  env: {
    es6: true,
    browser: true,
    node: true,
  ...
  rules: {
    ...
    'no-prototype-builtins': 0, // 禁止直接在对象实例上调用 Object.prototype 的一些方法。
    // 'no-template-curly-in-string': 2, // 禁止在常规字符串中出现模板字面量占位符语法
    // semi: ['off', 'always'], // 强制是否使用分号。
    ...
    // 'linebreak-style': [0, 'error', 'windows'], // 强制使用一致的换行符风格，用\n，不用\r\n
    ...
    // 'template-curly-spacing': 2, // 强制模板字符串中嵌入表达式周围空格的使用
    // 'sort-imports': [
    //   0,
    //   {
    //     ignoreCase: true,
