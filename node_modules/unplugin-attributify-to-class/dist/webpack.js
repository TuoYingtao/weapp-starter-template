...
__export(webpack_exports, {
  attributifyToClass: () => attributifyToClass,
      ...
      Array.from(templateCode.matchAll(elementRE)).forEach(([elementStr, valuedAttributeStr]) => {
        ...
        valuedAttributes.forEach(([sourceStr, name, _, content]) => {
          ...
          if (["class", "className"].includes(name)) {
            ...
            if (prefixedOnly && !name.startsWith(prefix))
              return;
            const attributifyToClass2 = content.split(splitterRE).filter(Boolean).map((v) => {
              if (v === "~")
            ...
            option.selectors.push(attributifyToClass2);
          }
        });
        result.push(option);
      });
...
// src/webpack.ts
var attributifyToClass = src_default.webpack;
