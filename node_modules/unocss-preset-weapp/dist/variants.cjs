...
const variantAria = {
  name: "aria",
  match(matcher, ctx) {
    const variant = utils.variantGetParameter("aria-", matcher, ctx.generator.config.separators);
    if (variant) {
...
function variantBreakpoints() {
  ...
  return {
    ...
    match(matcher, context) {
      const variantEntries = (colors.resolveBreakpoints(context) ?? []).map(({ point, size }, idx) => [point, size, idx]);
...
function scopeMatcher(name, combinator) {
  return {
    ...
    match(matcher, ctx) {
      ...
      let body = utils.variantGetBracket(`${name}-`, matcher, separators);
      if (!body) {
...
const variantContainerQuery = {
  ...
  match(matcher, ctx) {
    if (matcher.startsWith("@container"))
      return;
    const variant = utils.variantGetParameter("@", matcher, ctx.generator.config.separators);
    if (variant) {
...
const variantDataAttribute = {
  name: "data",
  match(matcher, ctx) {
    const variant = utils.variantGetParameter("data-", matcher, ctx.generator.config.separators);
    if (variant) {
      const [match, rest] = variant;
...
function taggedData(tagName) {
  return {
    name: `${tagName}-data`,
    match(matcher, ctx) {
      const variant = utils.variantGetParameter(`${tagName}-data-`, matcher, ctx.generator.config.separators);
      if (variant) {
        const [match, rest] = variant;
        const dataAttribute = colors.h.bracket(match) ?? ctx.theme.data?.[match] ?? "";
        if (dataAttribute) {
...
const variantPrint = utils.variantParentMatcher("print", "@media print");
const variantCustomMedia = {
  name: "media",
  match(matcher, ctx) {
    const variant = utils.variantGetParameter("media-", matcher, ctx.generator.config.separators);
    if (variant) {
      const [match, rest] = variant;
...
const variantSupports = {
  name: "supports",
  match(matcher, ctx) {
    const variant = utils.variantGetParameter("supports-", matcher, ctx.generator.config.separators);
    if (variant) {
      const [match, rest] = variant;
...
function variantColorsMediaOrClass(options = {}) {
  ...
  if (options?.dark === "class" || typeof options.dark === "object") {
    ...
    return [
      utils.variantMatcher("dark", (input) => ({ prefix: `${dark} $$ ${input.prefix}` }), transformRules),
      utils.variantMatcher("light", (input) => ({ prefix: `${light} $$ ${input.prefix}` }), transformRules)
    ];
  }
  return [
    utils.variantParentMatcher("dark", "@media (prefers-color-scheme: dark)", transformRules),
    utils.variantParentMatcher("light", "@media (prefers-color-scheme: light)", transformRules)
  ];
}

const variantLanguageDirections = [
  utils.variantMatcher("rtl", (input) => ({ prefix: `[dir="rtl"] $$ ${input.prefix}` })),
  utils.variantMatcher("ltr", (input) => ({ prefix: `[dir="ltr"] $$ ${input.prefix}` }))
];

const variantSelector = {
  name: "selector",
  match(matcher, ctx) {
    const variant = utils.variantGetBracket("selector-", matcher, ctx.generator.config.separators);
    if (variant) {
      const [match, rest] = variant;
...
const variantCssLayer = {
  name: "layer",
  match(matcher, ctx) {
    const variant = utils.variantGetParameter("layer-", matcher, ctx.generator.config.separators);
    if (variant) {
      const [match, rest] = variant;
...
const variantInternalLayer = {
  name: "uno-layer",
  match(matcher, ctx) {
    const variant = utils.variantGetParameter("uno-layer-", matcher, ctx.generator.config.separators);
    if (variant) {
      const [match, rest] = variant;
      const layer = colors.h.bracket(match) ?? match;
      if (layer) {
...
const variantScope = {
  name: "scope",
  match(matcher, ctx) {
    const variant = utils.variantGetBracket("scope-", matcher, ctx.generator.config.separators);
    if (variant) {
      const [match, rest] = variant;
...
const variantTheme = {
  ...
  match(matcher, ctx) {
    const newMatcher = utils$1.cacheRestoreSelector(matcher, ctx.theme.transformRules);
    if (!ruleUtils.hasThemeFn(newMatcher))
      return;
    return {
      matcher,
      handle(input, next) {
...
function negateMathFunction(value) {
  const match = value.match(colors.cssMathFnRE);
  if (match) {
...
function variantImportant() {
  ...
  return {
    ...
    match(matcher, ctx) {
      ...
      let base;
      matcher = utils$1.cacheRestoreSelector(matcher, ctx.theme?.transformRules);
...
function taggedPseudoClassMatcher(tag, parent, combinator) {
  ...
  const matchBracket = (input) => {
    const body = utils.variantGetBracket(`${tag}-`, input, []);
    if (!body)
      return;
...
function variantPseudoClassesAndElements() {
  ...
  return {
    ...
    match(input, ctx) {
      ...
      input = utils$1.cacheRestoreSelector(input, ctx.theme.transformRules);
...
exports.variantSpaceAndDivide = variantSpaceAndDivide;
