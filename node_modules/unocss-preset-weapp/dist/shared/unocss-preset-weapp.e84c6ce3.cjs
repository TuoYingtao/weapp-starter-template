...
const filters = require('./unocss-preset-weapp.640c88d4.cjs');
...
const animations = [
  ...
  [/^animate-(.+)$/, ([, name], { theme }) => {
    ...
    return { animation: colors.handler.bracket.cssvar(name) };
  }, { autocomplete: "animate-$animation.keyframes" }],
  [/^animate-name-(.+)/, ([, d]) => ({ "animation-name": colors.handler.bracket.cssvar(d) ?? d })],
  // timings
  [/^animate-duration-(.+)$/, ([, d], { theme }) => ({ "animation-duration": theme.duration?.[d || "DEFAULT"] ?? colors.handler.bracket.cssvar.time(d) }), { autocomplete: ["animate-duration", "animate-duration-$duration"] }],
  [/^animate-delay-(.+)$/, ([, d], { theme }) => ({ "animation-delay": theme.duration?.[d || "DEFAULT"] ?? colors.handler.bracket.cssvar.time(d) }), { autocomplete: ["animate-delay", "animate-delay-$duration"] }],
  [/^animate-ease(?:-(.+))?$/, ([, d], { theme }) => ({ "animation-timing-function": theme.easing?.[d || "DEFAULT"] ?? colors.handler.bracket.cssvar(d) }), { autocomplete: ["animate-ease", "animate-ease-$easing"] }],
  // fill mode
  [
    /^animate-(fill-mode-|fill-|mode-)?(.+)$/,
    ([, t, d]) => ["none", "forwards", "backwards", "both", ...[t ? colors.globalKeywords : []]].includes(d) ? { "animation-fill-mode": d } : void 0,
    {
      autocomplete: [
        "animate-(fill|mode|fill-mode)",
  ...
  [
    /^animate-(direction-)?(.+)$/,
    ([, t, d]) => ["normal", "reverse", "alternate", "alternate-reverse", ...[t ? colors.globalKeywords : []]].includes(d) ? { "animation-direction": d } : void 0,
    {
      autocomplete: [
        "animate-direction",
  ...
  // others
  [/^animate-(?:iteration-count-|iteration-|count-)(.+)$/, ([, d]) => ({ "animation-iteration-count": colors.handler.bracket.cssvar(d) ?? d.replace(/\-/g, ",") }), { autocomplete: ["animate-(iteration|count|iteration-count)", "animate-(iteration|count|iteration-count)-<num>"] }],
  [
    /^animate-(play-state-|play-|state-)?(.+)$/,
    ([, t, d]) => ["paused", "running", ...[t ? colors.globalKeywords : []]].includes(d) ? { "animation-play-state": d } : void 0,
    {
      autocomplete: [
        "animate-(play|state|play-state)",
  ...
  ...colors.makeGlobalStaticRules("animate", "animation")
];
...
function bgGradientToValue(cssColor) {
  if (cssColor)
    return colors.colorToString(cssColor, 0);
  return "rgba(255,255,255,0)";
}
function bgGradientColorValue(mode, cssColor, color, alpha) {
  if (cssColor) {
    if (alpha != null)
      return colors.colorToString(cssColor, alpha);
    else
      return colors.colorToString(cssColor, `var(--un-${mode}-opacity, ${colors.colorOpacityToString(cssColor)})`);
  }
  return colors.colorToString(color, alpha);
}
function bgGradientColorResolver() {
  return ([, mode, body], { theme }) => {
    const data = colors.parseColor(body, theme, "backgroundColor");
    if (!data)
      return;
...
function bgGradientPositionResolver() {
  return ([, mode, body]) => {
    return {
      [`--un-gradient-${mode}-position`]: `${Number(colors.handler.bracket.cssvar.percent(body)) * 100}%`
    };
  };
}
const backgroundStyles = [
  // gradients
  [/^bg-gradient-(.+)$/, ([, d]) => ({ "--un-gradient": colors.handler.bracket(d) }), {
  ...
  [/^(?:bg-gradient-)?stops-(\[.+\])$/, ([, s]) => ({ "--un-gradient-stops": colors.handler.bracket(s) })],
  [/^(?:bg-gradient-)?(from|via|to)-(.+)$/, bgGradientColorResolver()],
  [/^(?:bg-gradient-)?(from|via|to)-op(?:acity)?-?(.+)$/, ([, position, opacity]) => ({ [`--un-${position}-opacity`]: colors.handler.bracket.percent(opacity) })],
  ...
  [/^bg-gradient-to-([rltb]{1,2})$/, ([, d]) => {
    if (d in colors.positionMap) {
      return {
        "--un-gradient-shape": `to ${colors.positionMap[d]}`,
        "--un-gradient": "var(--un-gradient-shape), var(--un-gradient-stops)",
        "background-image": "linear-gradient(var(--un-gradient))"
      };
    }
  }, { autocomplete: `bg-gradient-to-(${Object.keys(colors.positionMap).filter((k) => k.length <= 2 && Array.from(k).every((c) => "rltb".includes(c))).join("|")})` }],
  [/^(?:bg-gradient-)?shape-(.+)$/, ([, d]) => {
    const v = d in colors.positionMap ? `to ${colors.positionMap[d]}` : colors.handler.bracket(d);
    if (v != null) {
      return {
        "--un-gradient-shape": v,
        "--un-gradient": "var(--un-gradient-shape), var(--un-gradient-stops)"
  ...
  }, { autocomplete: ["bg-gradient-shape", `bg-gradient-shape-(${Object.keys(colors.positionMap).join("|")})`, `shape-(${Object.keys(colors.positionMap).join("|")})`] }],
  ["bg-none", { "background-image": "none" }],
  ["box-decoration-slice", { "box-decoration-break": "slice" }],
  ["box-decoration-clone", { "box-decoration-break": "clone" }],
  ...colors.makeGlobalStaticRules("box-decoration", "box-decoration-break"),
  // size
  ...
  ...colors.globalKeywords.map((keyword) => [`bg-clip-${keyword}`, {
    "-webkit-background-clip": keyword,
    "background-clip": keyword
  }]),
  // positions
  // skip 1 & 2 letters shortcut
  [/^bg-([-\w]{3,})$/, ([, s]) => ({ "background-position": colors.positionMap[s] })],
  // repeats
  ...
  ...colors.makeGlobalStaticRules("bg-repeat", "background-repeat"),
  // origins
  ["bg-origin-border", { "background-origin": "border-box" }],
  ["bg-origin-padding", { "background-origin": "padding-box" }],
  ["bg-origin-content", { "background-origin": "content-box" }],
  ...colors.makeGlobalStaticRules("bg-origin", "background-origin")
];
...
const outline = [
  ...
  // color
  [/^outline-(?:color-)?(.+)$/, handleColorOrWidth$1, { autocomplete: "outline-$colors" }],
  // offset
...
function handleWidth$2([, b], { theme }) {
  return { "outline-width": theme.lineWidth?.[b] ?? colors.handler.bracket.cssvar.global.px(b) };
}
function handleColorOrWidth$1(match, ctx) {
  ...
  return colors.colorResolver("outline-color", "outline-color", "borderColor")(match, ctx);
}
...
function transformBorderColor(color, alpha, direction) {
  ...
  if (direction === "") {
    ...
    const result = colors.colorToString(color, `var(${opacityVar})`);
    if (result.includes(opacityVar))
      object[opacityVar] = typeof color === "string" ? 1 : colors.colorOpacityToString(color);
    ...
    const result = colors.colorToString(color, `var(${opacityDirectionVar})`);
...
function borderColorResolver(direction) {
  return ([, body], theme) => {
    const data = colors.parseColor(body, theme, "borderColor");
    if (!data)
      return;
    const { alpha, color, cssColor } = data;
    if (cssColor)
...
function handlerBorderSize([, a = "", b], { theme }) {
  const v = theme.lineWidth?.[b || "DEFAULT"] ?? colors.handler.bracket.cssvar.global.rpx(b || "1");
...
function handlerBorderColorOrSize([, a = "", b], ctx) {
  if (a in colors.directionMap) {
    if (colors.isCSSMathFn(colors.handler.bracket(b)))
      return handlerBorderSize(["", a, b], ctx);
    if (colors.hasParseableColor(b, ctx.theme, "borderColor")) {
      return Object.assign(
        {},
        ...colors.directionMap[a].map((i) => borderColorResolver(i)(["", b], ctx.theme))
      );
    }
  }
}
...
function handlerRounded([, a = "", s], { theme }) {
  ...
  const v = theme.borderRadius?.[s || "DEFAULT"] || colors.handler.bracket.cssvar.global.fraction.remToRpx(s || "1");
...
const bgColors = [
  [/^bg-(.+)$/, (params, body) => {
    ...
    if (bgUrlRE.test(d))
      return { "--un-url": colors.handler.bracket(d), "background-image": "var(--un-url)" };
    if (bgLengthRE.test(d) && colors.handler.bracketOfLength(d) != null)
      return { "background-size": colors.handler.bracketOfLength(d).split(" ").map((e) => colors.handler.fraction.auto.px.cssvar(e) ?? e).join(" ") };
    if ((colors.isSize(d) || bgPositionRE.test(d)) && colors.handler.bracketOfPosition(d) != null)
      return { "background-position": colors.handler.bracketOfPosition(d).split(" ").map((e) => colors.handler.position.fraction.auto.px.cssvar(e) ?? e).join(" ") };
    return colors.colorResolver("background-color", "bg", "backgroundColor")(params, body);
...
const divides = [
  ...
  // color & opacity
  [/^divide-(.+)$/, colors.colorResolver("border-color", "divide", "borderColor"), { autocomplete: "divide-$colors" }],
  [/^divide-op(?:acity)?-?(.+)$/, ([, opacity]) => ({ "--un-divide-opacity": colors.handler.bracket.percent(opacity) }), { autocomplete: ["divide-(op|opacity)", "divide-(op|opacity)-<percent>"] }],
  // styles
  ...borderStyles.map((style) => [`divide-${style}`, { "border-style": style }])
];
function handlerDivide([, d, s], { theme }) {
  let v = theme.lineWidth?.[s || "DEFAULT"] ?? colors.handler.bracket.cssvar.rpx(s || "1");
  if (v != null) {
    if (v === "0")
      v = "0px";
    const results = colors.directionMap[d].map((item) => {
...
const flex = [
  ...
  [/^flex-(.*)$/, ([, d], { theme }) => {
    ...
    return { flex: colors.handler.bracket(d) != null ? colors.handler.bracket(d).split(" ").map((e) => colors.handler.cssvar.fraction(e) ?? e).join(" ") : colors.handler.cssvar.fraction(d) };
  }],
  ["flex-1", { flex: "1 1 0%" }],
  ...
  [
    ...
    ([, d], { theme }) => {
      d = utils.cacheRestoreSelector(d, theme?.transformRules);
      return { "flex-basis": theme.spacing?.[d] ?? colors.handler.bracket.cssvar.auto.fraction.remToRpx(d) };
    },
...
const fonts = [
  ...
  // colors
  [/^(?:color|c)-(.+)$/, colors.colorResolver("color", "text", "textColor"), { autocomplete: "(color|c)-$colors" }],
  // style
  [/^(?:text|color|c)-(.+)$/, ([, v]) => colors.globalKeywords.includes(v) ? { color: v } : void 0, { autocomplete: `(text|color|c)-(${colors.globalKeywords.join("|")})` }],
  // opacity
  [/^(?:text|color|c)-op(?:acity)?-?(.+)$/, ([, opacity]) => ({ "--un-text-opacity": colors.handler.bracket.percent.cssvar(opacity) }), { autocomplete: "(text|color|c)-(op|opacity)-<percent>" }],
  // weights
  [
    /^(?:font|fw)-?([^-]+)$/,
    ([, s], { theme }) => ({ "font-weight": theme.fontWeight?.[s] || colors.handler.bracket.global.number(s) }),
    {
      autocomplete: [
  ...
  [
    /^(?:font-)?tracking-(.+)$/,
    ([, s], { theme }) => ({ "letter-spacing": theme.letterSpacing?.[s] || colors.handler.bracket.cssvar.global.remToRpx(s) }),
    { autocomplete: "tracking-$letterSpacing" }
  ],
  // word-spacing
  [
    /^(?:font-)?word-spacing-(.+)$/,
    ([, s], { theme }) => ({ "word-spacing": theme.wordSpacing?.[s] || colors.handler.bracket.cssvar.global.remToRpx(s) }),
...
const textIndents = [
  [
    ...
    ([, s], { theme }) => {
      s = utils.cacheRestoreSelector(s, theme?.transformRules);
      return { "text-indent": theme.textIndent?.[s || "DEFAULT"] || colors.handler.bracket.cssvar.global.fraction.remToRpx(s) };
    },
...
const textStrokes = [
  ...
  // colors
  [/^text-stroke-(.+)$/, colors.colorResolver("-webkit-text-stroke-color", "text-stroke", "borderColor"), { autocomplete: "text-stroke-$colors" }],
...
const textShadows = [
  ...
  // colors
  [/^text-shadow-color-(.+)$/, colors.colorResolver("--un-text-shadow-color", "text-shadow", "shadowColor"), { autocomplete: "text-shadow-color-$colors" }],
...
function handleThemeByKey(s, theme, key) {
  return theme[key]?.[s] || colors.handler.bracket.cssvar.global.remToRpx(s);
}
function handleSize([, s], { theme }) {
  const themed = core.toArray(theme.fontSize?.[s]);
  const size = themed?.[0] ?? (theme.whRpx ? colors.handler.bracket.cssvar.global.rpx(s) : colors.handler.bracket.cssvar.global.remToRpx(s));
  if (size != null)
...
function handlerColorOrSize(match, ctx) {
  ...
  return colors.colorResolver("color", "text", "textColor")(match, ctx);
}
function handleText([, s = "base"], { theme }) {
  const split = colors.splitShorthand(s, "length", theme);
  if (!split)
    return;
  ...
  const fontSize = colors.handler.bracketOfLength.remToRpx(size);
  ...
  if (theme.whRpx)
    return { "font-size": colors.handler.bracketOfLength.rpx(s) };
  else
    return { "font-size": colors.handler.bracketOfLength.remToRpx(s) };
}
...
function handleGap([, d = "", s], { theme }) {
  const v = theme.spacing?.[s] ?? colors.handler.bracket.cssvar.global.remToRpx(s);
  if (v != null) {
    return {
      [`${directions[d]}gap`]: v
    };
...
function autoDirection(c, theme, prop) {
  ...
  return colors.handler.bracket.cssvar.auto.remToRpx(prop);
}
const grids = [
  // displays
  ["grid", { display: "grid" }],
...
function handleInsetValue(v, { theme }) {
  ...
  return theme.spacing?.[v] ?? (theme?.whRpx ? colors.handler.bracket.cssvar.global.auto.fraction.rpx(v) : colors.handler.bracket.cssvar.global.auto.fraction.remToRpx(v));
}
...
const zIndexes = [
  ...
  [/^(?:position-|pos-)?z-(.+)$/, ([, v], { theme }) => ({ "z-index": theme.zIndex?.[v] ?? colors.handler.bracket.cssvar.global.auto.number(v) }), { autocomplete: "z-<num>" }]
];
const boxSizing = [
...
function getSizeValue(minmax, hw, theme, prop) {
  ...
  if (theme.whRpx)
    return colors.handler.bracket.cssvar.global.auto.fraction.rpx(prop);
  else
    return colors.handler.bracket.cssvar.global.auto.fraction.remToRpx(prop);
}
const sizes = [
...
function handleBreakpoint(context, point, key = "breakpoints") {
  const bp = colors.resolveBreakpoints(context, key);
  if (bp)
...
function handlerSpace([, d, s], { theme }) {
  let v = theme.spacing?.[s || "DEFAULT"] ?? colors.handler.bracket.cssvar.auto.fraction.remToRpx(s || "1");
  if (v != null) {
    if (v === "0")
      v = "0px";
    const results = colors.directionMap[d].map((item) => {
...
function handleWidth$1([, b], { theme }) {
  return { "text-decoration-thickness": theme.lineWidth?.[b] ?? colors.handler.bracket.cssvar.global.px(b) };
}
function handleColorOrWidth(match, ctx) {
  if (colors.isCSSMathFn(colors.handler.bracket(match[1])))
    return handleWidth$1(match, ctx);
  const result = colors.colorResolver("text-decoration-color", "line", "borderColor")(match, ctx);
  if (result) {
    return {
...
const svgUtilities = [
  // fills
  [/^fill-(.+)$/, colors.colorResolver("fill", "fill", "backgroundColor"), { autocomplete: "fill-$colors" }],
  ...
  // stroke colors
  [/^stroke-(.+)$/, colors.colorResolver("stroke", "stroke", "borderColor"), { autocomplete: "stroke-$colors" }],
...
function handleWidth([, b], { theme }) {
  ...
  return { "stroke-width": theme.lineWidth?.[b] ?? colors.handler.bracket.cssvar.fraction.px.number(b) };
}
...
const lineClamps = [
  ...
  ...["none", ...colors.globalKeywords].map((keyword) => [`line-clamp-${keyword}`, {
    "overflow": "visible",
    "display": "block",
...
const rules = [
  ...
  svgUtilities,
  borders,
  filters.contentVisibility,
  filters.contents,
  filters.contains,
  fonts,
  tabSizes,
  textIndents,
  filters.textOverflows,
  textDecorations,
  textStrokes,
  textShadows,
  filters.textTransforms,
  textAligns,
  filters.fontStyles,
  filters.fontSmoothings,
  filters.boxShadows,
  filters.rings,
  flex,
  grids,
  ...
  filters.contains,
  filters.textWraps,
  filters.dynamicViewportHeight,
  // should be the last
  // questionMark,
  // wind
  filters.backgroundBlendModes,
  filters.hyphens,
  filters.isolations,
  filters.mixBlendModes,
  filters.objectPositions,
  filters.screenReadersAccess,
  filters.textTransforms,
  filters.writingModes,
  filters.writingOrientations,
  spaces,
  lineClamps,
  filters.filters
].flat(1);
