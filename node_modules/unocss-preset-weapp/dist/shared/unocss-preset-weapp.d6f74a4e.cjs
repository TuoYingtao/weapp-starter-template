...
function directionSize(propertyPrefix) {
  return ([_, direction, size], { theme }) => {
    size = utils.cacheRestoreSelector(size, theme?.transformRules);
    const v = theme.spacing?.[size || "DEFAULT"] ?? handler.bracket.cssvar.global.auto.fraction.remToRpx(size);
    if (v != null)
...
function splitShorthand(body, type, theme) {
  const [front, rest] = ruleUtils.getStringComponent(utils.cacheRestoreSelector(body, theme.transformRules), "[", "]", ["/", ":", "_"]) ?? [];
  if (front != null) {
