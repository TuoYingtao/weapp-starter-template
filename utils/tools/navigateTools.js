...
export const navigateTo = (config = {}, type = "navigate") => {
	...
	return is_tab ? wx.switchTab({ ...config }) : type == "redirect" ? wx.redirectTo({ ...config }) : wx.navigateTo({ ...config });
};

/**
 * @method 页面返回
 */
export const navigateBack = (config = {}) => (getCurrentPages().length < 2 ? navigateTo(config, "redirect") : wx.navigateBack(config));
